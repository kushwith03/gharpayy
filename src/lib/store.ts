...
interface AppState {
  ...
  importLead: (input: ImportLeadInput) => Lead;
}
    ...
    (set, get) => ({
      ...
      importLead: (input) => {
        ...
        const initialLead: Lead = {
          ...input,
          assignedTcmId: "", // will be assigned below
          stage: "new",
          intent: "warm",
          confidence: 45,
          tags: [],
        ...
        // Use existing routing logic for auto-assignment
        const suggestion = autoAssignFn(initialLead, get().tcms, get().leads, get().tours);
        const lead: Lead = { ...initialLead, assignedTcmId: suggestion.tcmId };
        ...
        pushActivity(set, get, {
          ...
          actor: "flow-ops",
          leadId: lead.id,
          text: `Lead created and auto-assigned to ${getTcm(lead.assignedTcmId)?.name ?? lead.assignedTcmId} · ${suggestion.reasons.join(" · ")}`,
        });
